...
void archive_check_se_match_retrieve_text(
    ...
    match_align_input_t* const restrict match_align_input,
    mm_stack_t* const restrict mm_stack) {
  // Retrieve location
  locator_t* const restrict locator = archive->locator;
  const uint8_t* const restrict tag = (uint8_t*)match_trace->sequence_name;
  ...
  // Expand text-range
  locator_interval_t* const restrict locator_interval = locator_lookup_interval(archive->locator,index_position);
...
bool archive_check_se_match_check_optimum(
    ...
    swg_penalties_t* const restrict swg_penalties,
    match_align_input_t* const restrict match_align_input,
    match_trace_t* const restrict optimum_match_trace,
...
// match_trace_print(stream,matches->cigar_vector,);
  ...
  tab_global_dec();
  match_scaffold_t* const restrict match_scaffold = (match_scaffold_t*) match_trace->match_scaffold;
...
void archive_check_se_matches(
  ...
  FILE* const restrict stream = gem_error_get_stream();
  const char* const restrict sequence_buffer = string_get_buffer(&sequence->read);
  const uint64_t key_length = sequence_get_length(sequence);
  // Prepare
  uint8_t* const restrict key = mm_stack_calloc(mm_stack,key_length,uint8_t,false);
  uint64_t i;
