 ...
 * Constructor
 */
void sequence_init(sequence_t* const restrict sequence) {
...
void sequence_init_mm(sequence_t* const restrict sequence,mm_stack_t* const restrict mm_stack) {
...
void sequence_clear(sequence_t* const restrict sequence) {
...
void sequence_destroy(sequence_t* const restrict sequence) {
 ...
 * Accessors
 */
uint64_t sequence_get_length(sequence_t* const restrict sequence) {
  return string_get_length(&sequence->read);
}
char* sequence_get_tag(sequence_t* const restrict sequence) {
...
char* sequence_get_read(sequence_t* const restrict sequence) {
...
char* sequence_get_qualities(sequence_t* const restrict sequence) {
...
bool sequence_has_qualities(const sequence_t* const restrict sequence) {
  return !string_is_null(&sequence->qualities);
}
bool sequence_has_casava_tag(const sequence_t* const restrict sequence) {
  return !string_is_null(&sequence->casava_tag);
}
bool sequence_has_extra_tag(const sequence_t* const restrict sequence) {
  return !string_is_null(&sequence->extra_tag);
}
sequence_end_t sequence_get_end_info(const sequence_t* const restrict sequence) {
  return sequence->end_info;
}
/*
 * Utils
 */
bool sequence_equals(sequence_t* const restrict sequence_a,sequence_t* const restrict sequence_b) {
  return string_equals(&sequence_a->read,&sequence_b->read);
}
void sequence_generate_reverse(sequence_t* const restrict sequence,sequence_t* const restrict rev_sequence) {
  ...
  // Reverse Read
  int64_t pos;
  const char* const restrict seq_buffer = string_get_buffer(&sequence->read);
...
void sequence_generate_reverse_complement(sequence_t* const restrict sequence,sequence_t* const restrict rc_sequence) {
  // Prepare rc_string (Read)
  const uint64_t seq_buffer_length = string_get_length(&sequence->read);
 ...
 * Display
 */
void sequence_print(FILE* const restrict stream,sequence_t* const restrict sequence) {
