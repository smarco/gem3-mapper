...
void output_sam_print_opt_field_tag_RG(buffered_output_file_t* const buffered_output_file,string_t* const read_group) {
...
uint16_t output_sam_calculate_flag_se_match(
    ...
    const bool supplementary_alignment,const bool not_passing_QC,
...
uint16_t output_sam_calculate_flag_pe_map(
    ...
    const bool supplementary_alignment,const bool not_passing_QC,
    const bool PCR_duplicate,const bool paired) {
...
void output_sam_print_opt_field_tag_XA_match(
    buffered_output_file_t* const buffered_output_file,const matches_t* const matches,
    match_trace_t* const subdominant_match,const output_sam_parameters_t* const output_sam_parameters) {
...
void output_sam_print_core_fields_se(
    buffered_output_file_t* const buffered_output_file,sequence_t* const seq_read,
    const matches_t* const matches,match_trace_t* const match,const bool secondary_alignment,
    const bool supplementary_alignment,const bool not_passing_QC,const bool PCR_duplicate,
    const output_sam_parameters_t* const output_sam_parameters) {
  // (1) Print QNAME
  output_sam_print_qname(buffered_output_file,&seq_read->tag);
...
void output_sam_print_optional_fields_se(
  ...
  if (print_tag_XS && match_number+1 < num_matches) {
    const match_trace_t* const next_match_trace = matches_get_match_trace(matches,match_number+1);
