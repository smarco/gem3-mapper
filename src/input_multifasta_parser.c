...
GEM_INLINE void input_multifasta_parse_tag(input_file_t* const input_multifasta,string_t* const tag) {
  ...
  string_clear(tag);
  // Check empty tag
  gem_cond_fatal_error(!input_file_next_char(input_multifasta),MULTIFASTA_TAG_EMPTY,PRI_input_file_content(input_multifasta));
  // Read tag
  char current_char = input_file_get_current_char(input_multifasta);
  while (!IS_ANY_EOL(current_char) && !MFASTA_IS_ANY_TAG_SEPARATOR(current_char) && input_file_next_char(input_multifasta)) {
  ...
  if (!IS_ANY_EOL(current_char) && !MFASTA_IS_ANY_TAG_SEPARATOR(current_char)) string_append_char(tag,current_char);
  // Check empty and append EOS
  gem_cond_fatal_error(string_get_length(tag)==0,MULTIFASTA_TAG_EMPTY,PRI_input_file_content(input_multifasta)); // Empty TAG
