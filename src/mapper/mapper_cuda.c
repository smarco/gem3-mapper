...
//      mapper_cuda_search_t* const restrict mapper_cuda_search = g_mapper_cuda_search + i; // Thread
//      fprintf(stream,"GEM::Running-Thread (threadID = %"PRIu64")\n",mapper_cuda_search->thread_id);
//      // Display Input State
//      const sequence_t* const restrict sequence = archive_search_get_sequence(mapper_cuda_search->);
//      tab_global_inc();
 ...
 * Index loader
 */
void mapper_load_gpu_index(mapper_parameters_t* const restrict parameters) {
  TIMER_START(&parameters->loading_time);
  // Parameters
  mapper_parameters_cuda_t* const restrict cuda_parameters = &parameters->cuda;
  ...
  char* const restrict gpu_index_name = gem_strcat(parameters->io.index_file_name,".gpu");
...
void mapper_cuda_setup_thread(
    mapper_cuda_search_t* const restrict mapper_search,
...
void mapper_cuda_run(mapper_parameters_t* const restrict mapper_parameters,const bool paired_end) {
  // Check CUDA-Support
  if (!gpu_supported()) GEM_CUDA_NOT_SUPPORTED();
  // Parameters
  // Parameters
  mapper_parameters_cuda_t* const restrict cuda_parameters = &mapper_parameters->cuda;
  ...
  mapper_cuda_search_t* const restrict mapper_search = mm_malloc(num_threads*sizeof(mapper_cuda_search_t));
