...
void mapper_error_report(FILE* stream) {
  ...
  MUTEX_BEGIN_SECTION(mapper_error_report_mutex) {
    ...
    for (i=0;i<num_threads;++i) {
      mapper_search_t* const restrict mapper_search = g_mapper_searches + i; // Thread
      if (mapper_search->paired_end) {
        ...
        // Display Input State
        const sequence_t* const restrict sequence = archive_search_get_sequence(mapper_search->archive_search);
        tab_global_inc();
        ...
        fprintf(stream,"GEM::Running-Thread (threadID = %"PRIu64") currently processing\n",mapper_search->thread_id);
        // Display Input State
        const sequence_t* const restrict sequence_end1 = archive_search_get_sequence(mapper_search->archive_search_end1);
        const sequence_t* const restrict sequence_end2 = archive_search_get_sequence(mapper_search->archive_search_end2);
        tab_global_inc();
 ...
 * Mapper Parameters
 */
void mapper_parameters_set_defaults_io(mapper_parameters_io_t* const restrict io) {
...
void mapper_parameters_set_defaults_system(mapper_parameters_system_t* const restrict system) {
  /* System */
  const uint64_t num_processors = system_get_num_processors();
...
void mapper_parameters_set_defaults_cuda(mapper_parameters_cuda_t* const restrict cuda) {
  /* CUDA settings */
  const uint64_t num_processors = system_get_num_processors();
  /* CUDA */
...
void mapper_parameters_set_defaults_hints(mapper_parameters_hints_t* const restrict hints) {
  /* Hints */
}
void mapper_parameters_set_defaults_misc(mapper_parameters_misc_t* const restrict misc) {
  /* QC */
...
error_code_t mapper_SE_read_single_sequence(mapper_search_t* const restrict mapper_search) {
  const mapper_parameters_io_t* const restrict parameters_io = &mapper_search->mapper_parameters->io;
  sequence_t* const restrict sequence = archive_search_get_sequence(mapper_search->archive_search);
...
void mapper_PE_output_matches(
    ...
    mapping_stats_t* const restrict mstats) {
#ifdef MAPPER_OUTPUT
  switch (parameters->io.output_format) {
    case MAP:
 ...
 * SE Mapper
 */
void* mapper_SE_thread(mapper_search_t* const restrict mapper_search) {
  ...
  search_parameters_t* const restrict base_search_parameters = &mapper_search->mapper_parameters->base_search_parameters;
 ...
 * PE Mapper
 */
void* mapper_PE_thread(mapper_search_t* const restrict mapper_search) {
  // GEM-thread error handler
  gem_thread_register_id(mapper_search->thread_id+1);
 ...
 * SE/PE runnable
 */
void mapper_run(mapper_parameters_t* const restrict mapper_parameters,const bool paired_end) {
  // Load GEM-Index
  ...
  mapper_search_t* const restrict mapper_search = mm_calloc(num_threads,mapper_search_t,false); // Allocate mapper searches
  ...
  mapping_stats_t* const restrict mstats = mapper_parameters->global_mapping_stats ?
