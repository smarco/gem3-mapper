...
GEM_INLINE bool gpu_buffer_align_bpm_fits_in_buffer(
  ...
  const uint64_t max_candidates = gpu_buffer_align_bpm_get_max_candidates(gpu_buffer_align_bpm->buffer);
...
GEM_INLINE void gpu_buffer_align_bpm_pattern_compile(
    gpu_bpm_qry_entry_t* const query_pattern,const bpm_pattern_t* const bpm_pattern) {
  // Copy PEQ pattern
...
GEM_INLINE void gpu_buffer_align_bpm_add_pattern(gpu_buffer_align_bpm_t* const gpu_buffer_align_bpm,pattern_t* const pattern) {
  ...
  gpu_bpm_qry_info_t* query_info = gpu_bpm_buffer_get_peq_info_(gpu_buffer) + gpu_buffer_align_bpm->pattern_id;
  ...
  for (i=0;i<num_chunks;++i,++query_info) {
    query_info->posEntry = num_PEQ_entries + i*entries_per_chunk;
   ...
   */
  // Add pattern query/queries
  gpu_bpm_qry_entry_t* const query_pattern = gpu_bpm_buffer_get_peq_entries_(gpu_buffer) + num_PEQ_entries;
  gpu_buffer_align_bpm->num_PEQ_entries += num_entries;
  // Select computing device
  if (gpu_buffer_align_bpm->compute_cpu) {
    gpu_buffer_align_bpm_pattern_compile(query_pattern,bpm_pattern);
  } else {
    bpm_pattern_t** query_pattern_as_bpm_pattern = (bpm_pattern_t**)(&query_pattern->bitmap[0][0]);
    *query_pattern_as_bpm_pattern = bpm_pattern;
  }
}
...
//  void* const gpu_buffer = gpu_buffer_align_bpm->buffer;
//  gpu_bpm_qry_info_t* const query_info = gpu_bpm_buffer_get_peq_info_(gpu_buffer);
//  gpu_bpm_qry_entry_t* const query_pattern = gpu_bpm_buffer_get_peq_entries_(gpu_buffer);
//  gpu_bpm_cand_info_t* const query_candidate = gpu_bpm_buffer_get_candidates_(gpu_buffer);
//  gpu_bpm_alg_entry_t* const results_buffer = gpu_bpm_buffer_get_alignments_(gpu_buffer);
//
