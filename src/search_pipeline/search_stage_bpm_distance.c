 ...
 * Error Messages
 */
#define GEM_ERROR_SEARCH_STAGE_BPM_DISTANCE_UNPAIRED_QUERY \
  "Search-stage BPM-Distance. Couldn't retrieve query-pair"

/*
 * Internal Accessors
 */
search_stage_bpm_distance_buffer_t* search_stage_bpm_distance_get_buffer(
    search_stage_bpm_distance_t* const search_stage,
    const uint64_t buffer_pos) {
  return *vector_get_elm(search_stage->buffers,buffer_pos,search_stage_bpm_distance_buffer_t*);
}
search_stage_bpm_distance_buffer_t* search_stage_bpm_distance_get_current_buffer(
    search_stage_bpm_distance_t* const search_stage) {
  return search_stage_bpm_distance_get_buffer(search_stage,search_stage->iterator.current_buffer_idx);
}
/*
 * Setup
 */
search_stage_bpm_distance_t* search_stage_bpm_distance_new(
  ...
  // Alloc
  search_stage_bpm_distance_t* const search_stage = mm_alloc(search_stage_bpm_distance_t);
  ...
  for (i=0;i<num_buffers;++i) {
    search_stage_bpm_distance_buffer_t* const buffer_vc =
        search_stage_bpm_distance_buffer_new(gpu_buffer_collection,buffers_offset+i,bpm_distance_enabled);
    vector_insert(search_stage->buffers,buffer_vc,search_stage_bpm_distance_buffer_t*);
  }
  search_stage->iterator.num_buffers = num_buffers;
  search_stage_bpm_distance_clear(search_stage); // Clear buffers
  // Return
  return search_stage;
}
void search_stage_bpm_distance_clear(
  ...
  uint64_t i;
  for (i=0;i<num_buffers;++i) {
    search_stage_bpm_distance_buffer_clear(
...
void search_stage_bpm_distance_delete(
  ...
  uint64_t i;
  for (i=0;i<num_buffers;++i) {
    search_stage_bpm_distance_buffer_delete(
        search_stage_bpm_distance_get_buffer(search_stage,i));
  }
...
bool search_stage_bpm_distance_send_se_search(
  ...
  search_stage_bpm_distance_buffer_t* current_buffer =
      search_stage_bpm_distance_get_current_buffer(search_stage);
  while (!search_stage_bpm_distance_buffer_fits(current_buffer,archive_search,NULL)) {
    // Change group
    ...
    if (search_stage->iterator.current_buffer_idx < last_buffer_idx) {
      ...
      search_stage_bpm_distance_buffer_send(current_buffer);
      // Next buffer
      ++(search_stage->iterator.current_buffer_idx);
      current_buffer = search_stage_bpm_distance_get_current_buffer(search_stage);
    } else {
      return false;
    }
  }
  // Add SE Search
  search_stage_bpm_distance_buffer_add(current_buffer,archive_search);
  // Copy candidates to the buffer
  archive_search_se_stepwise_bpm_distance_copy(archive_search,current_buffer->gpu_buffer_bpm_distance);
  // Return ok
  return true;
}
bool search_stage_bpm_distance_send_pe_search(
  ...
  search_stage_bpm_distance_buffer_t* current_buffer =
      search_stage_bpm_distance_get_current_buffer(search_stage);
  while (!search_stage_bpm_distance_buffer_fits(current_buffer,archive_search_end1,archive_search_end2)) {
    // Change group
    const uint64_t last_buffer_idx = search_stage->iterator.num_buffers - 1;
    if (search_stage->iterator.current_buffer_idx < last_buffer_idx) {
      // Send the current group to verification
  ...
  // Add PE Search
  search_stage_bpm_distance_buffer_add(current_buffer,archive_search_end1);
  search_stage_bpm_distance_buffer_add(current_buffer,archive_search_end2);
  // Copy candidates to the buffer
  archive_search_se_stepwise_bpm_distance_copy(archive_search_end1,current_buffer->gpu_buffer_bpm_distance);
  archive_search_se_stepwise_bpm_distance_copy(archive_search_end2,current_buffer->gpu_buffer_bpm_distance);
  // Return ok
  return true;
}
/*
 * Retrieve operators
 */
void search_stage_bpm_distance_retrieve_begin(search_stage_bpm_distance_t* const search_stage) {
  ...
  PROF_ADD_COUNTER(GP_SSTAGE_BPM_DISTANCE_BUFFERS,iterator->current_buffer_idx+1);
  ...
  // Fetch first group
  search_stage_bpm_distance_buffer_receive(current_buffer);
}
...
bool search_stage_bpm_distance_retrieve_next(
    search_stage_bpm_distance_t* const search_stage,
    search_stage_bpm_distance_buffer_t** const current_buffer,
    archive_search_t** const archive_search) {
  // Check state
  if (search_stage->search_stage_mode == search_group_buffer_phase_sending) {
    search_stage_bpm_distance_retrieve_begin(search_stage);
  }
  ...
  *current_buffer = search_stage_bpm_distance_get_current_buffer(search_stage);
  ...
  // Retrieve Search
  search_stage_bpm_distance_buffer_retrieve(*current_buffer,iterator->current_search_idx,archive_search);
 ...
 */
void search_stage_bpm_distance_prepare(
    archive_search_t* const archive_search,
    search_pipeline_handlers_t* const search_pipeline_handlers,
...
bool search_stage_bpm_distance_retrieve_se_search(
  ...
  // Retrieve next
  search_stage_bpm_distance_buffer_t* current_buffer;
  const bool success = search_stage_bpm_distance_retrieve_next(search_stage,&current_buffer,archive_search);
...
bool search_stage_bpm_distance_retrieve_pe_search(
   ...
   */
  // Retrieve next (End/2)
  success = search_stage_bpm_distance_retrieve_next(search_stage,&current_buffer,archive_search_end2);
  gem_cond_fatal_error(!success,SEARCH_STAGE_BPM_DISTANCE_UNPAIRED_QUERY);
  // Prepare Archive Search
  search_stage_bpm_distance_prepare(*archive_search_end2,
