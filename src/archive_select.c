...
GEM_INLINE void archive_select_realign_match(
    ...
    search_actual_parameters_t* const search_parameters,select_parameters_t* const select_parameters,
    matches_t* const matches,match_trace_t* const match_trace,
    const uint8_t* const text,const uint64_t length,const uint64_t edit_distance) {
...
GEM_INLINE void archive_select_decode_matches(
  ...
  search_actual_parameters_t* const search_parameters = &archive_search->search_actual_parameters;
  select_parameters_t* const select_parameters = archive_search->select_parameters;
  text_collection_t* const text_collection = mm_search->text_collection;
  ...
  VECTOR_ITERATE(matches->global_matches,match_trace,match_trace_num,match_trace_t) {
    if (match_trace->distance <= last_stratum_distance) {
      ...
      if (global_matches_it != match_trace) {
        // Store it in the vector (removing unwanted ones)
        *global_matches_it = *match_trace;
        ...
        // match_trace->match_trace_type == match_trace_type_mark_hamming
        // match_trace->match_trace_type == match_trace_type_mark_edit
        archive_select_realign_match(archive,search_parameters,select_parameters,
            matches,match_trace,text_trace->text,match_trace->match_trace_length,match_trace->distance);
      }
      ++global_matches_it;
    }
  }
  ...
  VECTOR_ITERATE(matches->interval_matches,match_interval,match_interval_num,match_interval_t) {
    ...
    // Realign (if needed)
    archive_select_realign_match(archive,search_parameters,select_parameters,
        matches,&match_trace,match_interval->text,match_interval->length,match_interval->distance);
...
GEM_INLINE void archive_select_matches(
  ...
  if (strata_to_decode > 0) {
    archive_select_decode_matches(archive_search,matches,
        strata_to_decode,matches_to_decode_last_stratum,mm_search);
    // Sort all matches
