...
input_file_sliced_t* input_stream_sliced_open(
    ...
    const uint64_t block_size) {
  // Allocate handler
  input_file_sliced_t* const restrict input_file_sliced = mm_alloc(input_file_sliced_t);
  // Initialize
...
void input_file_sliced_close(input_file_sliced_t* const restrict input_file_sliced) {
...
char* input_file_sliced_get_file_name(input_file_sliced_t* const restrict input_file_sliced) {
  char* const restrict file_name = fm_get_file_name(input_file_sliced->file_manager);
 ...
 * Buffer Request
 */
input_buffer_t* input_file_sliced_request_buffer_empty(input_file_sliced_t* const restrict input_file_sliced) {
...
void input_file_sliced_input_buffer_read(
    input_file_sliced_t* const restrict input_file_sliced,
    input_buffer_t* const restrict input_buffer) {
...
void input_file_sliced_process(input_file_sliced_t* const restrict input_file_sliced) {
  ...
  while (true) {
    ...
    input_buffer_t* const restrict input_buffer = input_file_sliced_request_buffer_empty(input_file_sliced);
...
void input_file_sliced_discard_exhausted_buffers(
  ...
  for (i=0;i<num_buffers;++i) {
    input_buffer_t* const restrict input_buffer = *(vector_get_elm(input_buffers,i,input_buffer_t*));
...
bool input_file_sliced_read_lines(
  ...
  // Parameters
  const uint64_t total_lines = vector_get_used(current_file_buffer->line_lengths) - 1;
  uint32_t* const restrict line_lengths = vector_get_mem(current_file_buffer->line_lengths,uint32_t);
