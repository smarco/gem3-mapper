 ...
 * Setup
 */
void output_sam_parameters_set_defaults(output_sam_parameters_t* const restrict sam_parameters) {
  /* Header & RG  */
...
void output_sam_print_header(
  ...
  while (i<num_intervals) {
    ...
    const char* const restrict tag = locator_interval_get_tag(locator,intervals+i);
...
void output_sam_print_reduced_cigar_element(
    buffered_output_file_t* const restrict buffered_output_file,
    const cigar_element_t* const restrict cigar_element,
    uint64_t* const restrict matching) {
...
void output_sam_print_md_cigar_element(
    ...
    uint64_t* const restrict text_pos,
    const strand_t match_strand,
    const cigar_element_t* const restrict cigar_element,
    uint64_t* const restrict matching_length) {
...
void output_sam_print_opt_field_tag_MD(
  ...
  const cigar_element_t* const restrict cigar_array =
  ...
  // Print MD CIGAR
  const uint8_t* const restrict text = match_trace->text;
...
void output_sam_print_opt_field_tag_RG(
    buffered_output_file_t* const restrict buffered_output_file,
    string_t* const restrict read_group) {
...
uint16_t output_sam_calculate_flag_se_match(
    const match_trace_t* const restrict match,
    const bool secondary_alignment,
    const bool supplementary_alignment,
    const bool not_passing_QC,
    const bool PCR_duplicate) {
...
uint16_t output_sam_calculate_flag_pe_map(
    const match_trace_t* const restrict match,
    const match_trace_t* const restrict mate_match,
...
void output_sam_print_opt_field_tag_X5(
  ...
  if (gem_expect_false(matches_get_num_match_traces(matches)==0)) {
    ...
    VECTOR_ITERATE(matches->position_matches,match_trace,match_number,match_trace_t) {
      ...
      // Print Match
      output_map_print_match(buffered_output_file,matches,match_trace,true,map_format_v2);
    }
  }
}
...
void output_sam_print_opt_field_tag_XA_match(
    buffered_output_file_t* const restrict buffered_output_file,
    const matches_t* const restrict matches,
    match_trace_t* const restrict subdominant_match,
    const output_sam_parameters_t* const restrict output_sam_parameters) {
...
void output_sam_print_opt_field_tag_XA_pe_end1(
  ...
  if (num_matches > 1) {
    matches_t* const restrict matches = paired_matches->matches_end1;
...
void output_sam_print_opt_field_tag_XA_pe_end2(
  ...
  const uint64_t num_matches = paired_matches_get_num_maps(paired_matches);
  if (num_matches > 1) {
    matches_t* const restrict matches = paired_matches->matches_end2;
    paired_map_t* paired_map = paired_matches_get_maps(paired_matches);
    ++paired_map; // Skip primary
    // Reserve
    buffered_output_file_reserve(buffered_output_file,6);
...
void output_sam_print_qname(
    buffered_output_file_t* const restrict buffered_output_file,
    string_t* const restrict read) {
  const char* const restrict tag_buffer = string_get_buffer(read);
...
void output_sam_print_seq__qualities(
  ...
  if (secondary_alignment && output_sam_parameters->omit_secondary_read__qualities) {
    ...
    string_t* const restrict read = &seq_read->read;
    string_t* const restrict qualities = &seq_read->qualities;
    ...
    if (!string_is_null(read) && !string_is_null(qualities)) {
      ...
      if (match==NULL || match->strand==Forward) {
        ...
        int64_t i;
        bofprintf_char(buffered_output_file,'\t');
        const char* const restrict read_buffer = string_get_buffer(read);
        for (i=read_length-1;i>=0;--i) {
          bofprintf_char(buffered_output_file,dna_complement(read_buffer[i]));
        }
        bofprintf_char(buffered_output_file,'\t');
        const char* const restrict qualities_buffer = string_get_buffer(qualities);
...
void output_sam_print_core_fields_pe(
    ...
    sequence_t* const restrict seq_read,
    const matches_t* const restrict matches,
    match_trace_t* const restrict match,
    match_trace_t* const restrict mate,
...
void output_sam_print_optional_fields_se(
    ...
    const output_sam_parameters_t* const restrict sam_parameters) {
  /*
  ...
  // AS
  const alignment_model_t alignment_model = archive_search->search_parameters.alignment_model;
  ...
  if (print_tag_XS && match_number+1 < num_matches) {
    const match_trace_t* const restrict next_match_trace = matches_get_match_trace(matches,match_number+1);
...
void output_sam_print_optional_fields_pe(
    buffered_output_file_t* const restrict buffered_output_file,
    archive_search_t* const restrict archive_search,
    const paired_matches_t* const restrict paired_matches,
    const sequence_end_t sequence_end,
    const match_trace_t* const restrict match_trace,
    const uint64_t match_number,
    const bool print_tag_XS,
   ...
   */
  const uint64_t num_matches = paired_matches_get_num_maps(paired_matches);
  matches_t* const restrict matches = (sequence_end==paired_end1) ? paired_matches->matches_end1 : paired_matches->matches_end2;
  // RG
  if (sam_parameters->read_group_id) {
    output_sam_print_opt_field_tag_RG(buffered_output_file,sam_parameters->read_group_id);
  }
...
void output_sam_paired_end_matches_unpaired(
    ...
    const sequence_end_t sequence_end,
    matches_t* const restrict matches,
    match_trace_t* const restrict primary_match_mate,
    const output_sam_parameters_t* const restrict output_sam_parameters) {
  const bool supplementary_alignment = false;   // TODO
  const bool not_passing_QC = false;   // TODO
  const bool PCR_duplicate = false;  // TODO
...
void output_sam_paired_end_matches(
  ...
  if (gem_expect_false(!paired_matches_is_mapped(paired_matches))) {
    ...
    matches_t* const restrict matches_end1 = paired_matches->matches_end1;
    matches_t* const restrict matches_end2 = paired_matches->matches_end2;
    const uint64_t num_matches_end1 = vector_get_used(matches_end1->position_matches);
    const uint64_t num_matches_end2 = vector_get_used(matches_end2->position_matches);
    match_trace_t* const restrict primary_match_end1 = num_matches_end1 ? vector_get_mem(matches_end1->position_matches,match_trace_t) : NULL;
    match_trace_t* const restrict primary_match_end2 = num_matches_end2 ? vector_get_mem(matches_end2->position_matches,match_trace_t) : NULL;
    // End1
