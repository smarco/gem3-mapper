...
void matches_cigar_buffer_add_cigar_element__attr(
    cigar_element_t** const restrict cigar_buffer_sentinel,
...
void matches_cigar_vector_append_insertion(
    ...
    uint64_t* const restrict current_cigar_length,
  ...
  cigar_element_t* const restrict cigar_element = vector_get_free_elm(cigar_vector,cigar_element_t);// Add CIGAR element
...
void matches_cigar_vector_append_cigar_element(
    vector_t* const restrict cigar_vector,
    uint64_t* const restrict cigar_length,
    cigar_element_t* const restrict cigar_element) {
...
void matches_cigar_reverse(
  ...
  // Reverse CIGAR
  cigar_element_t* const restrict cigar_buffer = vector_get_elm(cigar_vector,cigar_buffer_offset,cigar_element_t);
  const uint64_t middle_point = cigar_length/2;
  uint64_t i;
  for (i=0;i<middle_point;++i) {
    cigar_element_t* const restrict origin = cigar_buffer + i;
    cigar_element_t* const restrict flipped = cigar_buffer + (cigar_length-1-i);
    SWAP(*origin,*flipped);
  ...
  if (cigar_length%2) {
    cigar_element_t* const restrict middle = cigar_buffer + middle_point;
...
uint64_t matches_cigar_compute_event_distance(
  ...
  const cigar_element_t* const restrict cigar_buffer = vector_get_elm(cigar_vector,cigar_buffer_offset,cigar_element_t);
...
int64_t matches_cigar_element_effective_length(const cigar_element_t* const restrict cigar_element) {
  switch (cigar_element->type) {
    case cigar_match:
...
int matches_cigar_cmp(
    vector_t* const restrict cigar_vector_match0,
    match_trace_t* const restrict match0,
    vector_t* const restrict cigar_vector_match1,
    match_trace_t* const restrict match1) {
  ...
  // Locate CIGARs
  cigar_element_t* const restrict match0_cigar = vector_get_elm(cigar_vector_match0,match0->match_alignment.cigar_offset,cigar_element_t);
  cigar_element_t* const restrict match1_cigar = vector_get_elm(cigar_vector_match1,match1->match_alignment.cigar_offset,cigar_element_t);
  // Compare
  uint64_t i;
...
void match_cigar_print(
  ...
  for (j=0;j<cigar_length;++j) {
    cigar_element_t* const restrict cigar_element = vector_get_elm(cigar_vector,cigar_buffer_offset+j,cigar_element_t);
    switch (cigar_element->type) {
