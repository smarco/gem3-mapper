...
void match_align_swg_add_region_matching(
    const region_matching_t* const restrict region_matching,
  ...
  // Parameters
  uint8_t* const restrict key = align_input->key;
    ...
    case region_matching_approximate:
      if (region_matching->cigar_length > 0) {
        ...
        for (i=0;i<region_matching->cigar_length;++i) {
          cigar_element_t* const restrict scaffolding_elm = vector_get_elm(cigar_vector,cigar_buffer_offset+i,cigar_element_t);
...
void match_align_swg_chain_scaffold(
  ...
  match_alignment_t* const restrict match_alignment = &match_trace->match_alignment;
  region_matching_t* const restrict scaffold_regions = match_scaffold->scaffold_regions;
  ...
  const region_matching_t* const restrict first_region_matching = scaffold_regions;
  ...
  for (i=1;i<num_scaffold_regions;++i) {
    // Align the gap
    const region_matching_t* const restrict prev_region_matching = scaffold_regions + (i-1);
    const region_matching_t* const restrict current_region_matching = scaffold_regions + i;
  ...
  const region_matching_t* const restrict last_region_matching = scaffold_regions + (num_scaffold_regions-1);
