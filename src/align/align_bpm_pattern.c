...
#define BMP_W64_SIZE   UINT64_SIZE
...
bpm_pattern_t* bpm_pattern_compile(
  ...
  const uint64_t pattern_num_words64 = DIV_CEIL(pattern_length,BMP_W64_LENGTH);
  const uint64_t PEQ_length = pattern_num_words64*BMP_W64_LENGTH;
  const uint64_t pattern_mod = pattern_length%BMP_W64_LENGTH;
  // Init fields
  ...
  // Allocate memory
  const uint64_t aux_vector_size = pattern_num_words64*BMP_W64_SIZE;
  ...
  for (i=0;i<pattern_length;++i) {
    ...
    const uint64_t block = i/BMP_W64_LENGTH;
    const uint64_t mask = 1ull<<(i%BMP_W64_LENGTH);
  ...
  for (i=0;i<top;++i) {
    ...
    bpm_pattern->init_score[i] = BMP_W64_LENGTH;
    bpm_pattern->pattern_left[i] = pattern_left;
    pattern_left = (pattern_left > BMP_W64_LENGTH) ? pattern_left-BMP_W64_LENGTH : 0;
  }
  ...
  if (pattern_mod > 0) {
    ...
    bpm_pattern->init_score[top] = BMP_W64_LENGTH;
  }
  // GPU Compile
...
bpm_pattern_t* bpm_pattern_compile_tiles(
  ...
  const uint64_t num_tiles = DIV_CEIL(bpm_pattern->pattern_num_words64,words64_per_tile);
  const uint64_t tile_length = words64_per_tile * UINT64_LENGTH;
  // Alloc
  ...
  for (i=0;i<num_tiles;++i) {
    ...
    bpm_pattern_tile->pattern_num_words64 = DIV_CEIL(actual_tile_length,BMP_W64_LENGTH);
    bpm_pattern_tile->pattern_mod = actual_tile_length % BMP_W64_LENGTH;
