 ...
 * Setup
 */
void string_init_(string_t* const restrict string,char* const restrict buffer,const uint64_t length,mm_stack_t* const restrict mm_stack) {
  // Initialize
  if (buffer!=NULL) {
    // Static String
...
void string_resize(string_t* const restrict string,const uint64_t length) {
  ...
  if (string->allocated < new_buffer_size) {
    if (string->mm_stack!=NULL) {
      const char* const restrict old_buffer = string->buffer;
 ...
 * Accessors
 */
void string_set_buffer(string_t* const restrict string,char* const restrict buffer,const uint64_t length) {
...
void string_left_append_buffer(string_t* const restrict string,const char* const restrict buffer,const uint64_t length) {
  ...
  char* const restrict string_buffer = string->buffer;
  int64_t i;
...
void string_left_append_string(string_t* const restrict string_dst,string_t* const restrict string_src) {
  ...
  // Shift dst characters to the left
  char* const restrict buffer_src = string_src->buffer;
  char* const restrict buffer_dst = string_dst->buffer;
  int64_t i;
...
void string_copy_reverse(string_t* const restrict string_dst,string_t* const restrict string_src) {
  ...
  // Reverse Read
  int64_t pos;
  const char* const restrict buffer = string_get_buffer(string_src);
...
int64_t string_cmp(string_t* const restrict string_a,string_t* const restrict string_b) {
  char* const restrict buffer_a = string_a->buffer;
  char* const restrict buffer_b = string_b->buffer;
...
string_t* string_dup(string_t* const restrict string) {
  string_t* const restrict string_cpy = mm_alloc(string_t);
  // Duplicate
...
char* gem_strndup(const char* const restrict buffer,const uint64_t length) {
  char* const restrict buffer_cpy = mm_malloc(length+1);
...
char* gem_strcat(const char* const restrict buffer_a,const char* const restrict buffer_b) {
  ...
  char* const restrict buffer_dst = mm_malloc(total_length+1);
  buffer_dst[0] = EOS;
...
void gem_strrev(char* const restrict buffer,const uint64_t length) {
  const uint64_t last_idx = length-1;
  const uint64_t middle = length/2;
  uint64_t i;
  for (i=0;i<middle;++i) buffer[i] = buffer[last_idx-i];
}
void gem_encrev(uint8_t* const restrict buffer,const uint64_t length) {
  const uint64_t last_idx = length-1;
  const uint64_t middle = length/2;
  uint64_t i;
  for (i=0;i<middle;++i) buffer[i] = buffer[last_idx-i];
