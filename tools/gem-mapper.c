...
#pragma omp parallel for private(idBuffer)
  for (idBuffer = 0; idBuffer < numBuffers; idBuffer++) {
    ...
    sendMyersBuffer(buffer[idBuffer], distance,
        testData[idBuffer].totalQueriesEntries, testData[idBuffer].numQueries,
        testData[idBuffer].numCandidates);
  ...
  if (error != 0) {
    fprintf(stderr, "Error %d, saving results \n", error);
    exit(EXIT_FAILURE);
  }
