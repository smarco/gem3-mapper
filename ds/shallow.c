...
void shallow_sort(Int32 *a, int n, int shallow_limit) {
  ...
  case (1):
    shallow_mkq(a, n, Text + 2);
    break;
  case (2):
    shallow_mkq16(a, n, Text + 2);
    break;
  case (4):
    shallow_mkq32(a, n, Text + 2);
    break;
  default:
    fprintf(stderr, "Invalid word size for mkqs (%d) (shallow_sort)\n",
        _ds_Word_size);
    exit(1);
  }
}
...
__inline__ Int32 cmp_unrolled_shallow_lcp(UChar *b1, UChar *b2) {
...
static void shallow_inssort_lcp(Int32 *a, Int32 n, UChar *text_depth) {
  ...
  for (i = 0; i < n; i++)
    lcp[i] = 0;     // I think this loop is not necessary
  ...
  for (i = 1; i < n; i++) {
    ...
    while (1) {
      ...
      r = cmp_unrolled_shallow_lcp(lcpi + a[j1] + text_depth,
          lcpi + text_depth_ai);
