...
region_matching_t* match_scaffold_compose_add_matching_exact(
    uint8_t* const text,uint64_t* const key_offset,
    uint64_t* const text_offset,const uint64_t cigar_offset,
    const uint64_t match_length,match_scaffold_t* const match_scaffold);
void match_scaffold_compose_add_mismatch(
    uint64_t* const key_offset,uint64_t* const text_offset,
    const uint64_t match_length,match_scaffold_t* const match_scaffold,
    region_matching_t* const last_scaffold_region);
void match_scaffold_compose_add_homopolymer(
    uint8_t* const text,uint64_t* const key_offset,
    uint64_t* const text_offset,const uint64_t context_cigar_offset,
    const uint64_t context_match_length,const cigar_t indel_type,
    const uint64_t indel_length,match_scaffold_t* const match_scaffold,
    region_matching_t* const last_scaffold_region);
