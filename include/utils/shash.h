...
void shash_clear(shash_t* const restrict shash);
void shash_delete(shash_t* const restrict shash);
...
void shash_remove_element(shash_t* const restrict shash,char* const restrict key);
...
#define GT_SHASH_BEGIN_ITERATE(shash,it_skey,it_element,type) { \
  ...
  HASH_ITER(hh,shash,shash_##sh_element,shash_##tmp) { \
    type* const restrict it_element = (type*)(shash_##sh_element->element); \
    char* const restrict it_skey = shash_##sh_element->key;
