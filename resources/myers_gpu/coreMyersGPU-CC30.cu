...
inline __device__ void shuffle_collaborative_shift_CC30(uint32_t value_A, uint32_t value_B, uint32_t value_C, uint32_t value_D,  
...
inline __device__ void shuffle_collaborative_sum_CC30(const uint32_t a_A, const uint32_t a_B, const uint32_t a_C, const uint32_t a_D, 
...
inline __device__ uint32_t selectEq_CC30(const uint32_t indexBase, 
...
inline __device__ uint32_t select_CC30(const uint32_t indexWord, 
...
__device__ void myerslocalKeplerKernel_CC30( const d_qryEntry_t *d_queries, const uint32_t * d_reference, const candInfo_t *d_candidates, 
		...
		if((positionRef < sizeRef) && ((sizeRef - positionRef) > sizeCandidate)){
			...
			for(idEntry = 0; idEntry < numEntriesPerCandidate; idEntry++){
				...
				for(intraBase = 0; intraBase < REFERENCE_CHARS_PER_ENTRY; intraBase++){
					...
					Eq_A = selectEq_CC30(indexBase, Eq0.x, Eq1.x, Eq2.x, Eq3.x, Eq4.x);
					Eq_B = selectEq_CC30(indexBase, Eq0.y, Eq1.y, Eq2.y, Eq3.y, Eq4.y);
					Eq_C = selectEq_CC30(indexBase, Eq0.z, Eq1.z, Eq2.z, Eq3.z, Eq4.z);
					Eq_D = selectEq_CC30(indexBase, Eq0.w, Eq1.w, Eq2.w, Eq3.w, Eq4.w);
					...
					shuffle_collaborative_sum_CC30(tEq_A, tEq_B, tEq_C, tEq_D, Pv_A, Pv_B, Pv_C, Pv_D, 
											  	   intraQueryThreadIdx, 
					...
					PH = select_CC30(indexWord, Ph_A, Ph_B, Ph_C, Ph_D);
					MH = select_CC30(indexWord, Mh_A, Mh_B, Mh_C, Mh_D);
					score += (((PH & mask) != 0) - ((MH & mask) != 0));

					shuffle_collaborative_shift_CC30(Ph_A, Ph_B, Ph_C, Ph_D, 
													 intraQueryThreadIdx,
													 &Ph_A, &Ph_B, &Ph_C, &Ph_D);
					shuffle_collaborative_shift_CC30(Mh_A, Mh_B, Mh_C, Mh_D, 
													 intraQueryThreadIdx,
													 &Mh_A, &Mh_B, &Mh_C, &Mh_D);
...
__global__ void myersKeplerKernel_CC30(const d_qryEntry_t *d_queries, const uint32_t * d_reference, const candInfo_t *d_candidates, const uint32_t *d_reorderBuffer, 
						    		   ...
						    		   uint32_t *d_initPosPerBucket, uint32_t *d_initWarpPerBucket, uint32_t numWarps)
{
	...
	myerslocalKeplerKernel_CC30(d_queries, d_reference, d_candidates, d_reorderBuffer, d_reorderResults, d_qinfo,
	 				 			idCandidate, sizeRef, numReorderedResults, intraQueryThreadIdx, threadsPerQuery);
}

extern "C"
myersError_t processMyersBufferOnKepler1stGen(buffer_t *mBuff)
{
	...
	//printf("KEPLER 1stGen: LAUNCH KERNEL -- Bloques: %d - Th_block %d\n", blocksPerGrid, threadsPerBlock);
	myersKeplerKernel_CC30<<<blocksPerGrid, threadsPerBlock, 0, idStream>>>((d_qryEntry_t *)qry->d_queries, ref->d_reference[idSupDev], cand->d_candidates, rebuff->d_reorderBuffer,
