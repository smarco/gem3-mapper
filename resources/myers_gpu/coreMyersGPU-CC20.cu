											...
											const uint32_t intraQueryThreadIdx, const uint32_t threadsPerQuery)
{
	if (idCandidate < numReorderedResults){
		...
		__shared__
		uint32_t globalInterBuff[(WARP_SIZE + 1) * (CUDA_NUM_THREADS/WARP_SIZE)];
		uint32_t *localInterBuff = globalInterBuff + ((threadIdx.x/WARP_SIZE) * (WARP_SIZE + 1));
		uint32_t intraWarpIdx = threadIdx.x % WARP_SIZE;
		...
		if((positionRef < sizeRef) && ((sizeRef - positionRef) > sizeCandidate)){
			...
			for(idColumn = 0; idColumn < sizeCandidate; idColumn++){
				...
				if((idColumn % REFERENCE_CHARS_PER_ENTRY) == 0){
						...
						candidate = funnelShiftL_CC20(currentCandidateEntry, lastCandidateEntry, candidateAlignment);
...
myersError_t processMyersBufferOnFermi(buffer_t *mBuff)
{
	...
	//We use 2-Dimensional Grid (because Fermi Arch specs are limited to 65535 Blocks per dim)
	uint32_t threadsPerBlock = CUDA_NUM_THREADS;
	uint32_t maxBlocksPerRow = 65535;
	uint32_t numThreads = rebuff->numWarps * WARP_SIZE;
	
	uint32_t numBlocks = DIV_CEIL(numThreads, threadsPerBlock);
	uint32_t rowsPerGrid = DIV_CEIL(numBlocks, maxBlocksPerRow);
	...
	//printf("FERMI: LAUNCH KERNEL -- blocksPerRow: %d - rowsPerGrid %d - threadsPerBlock %d\n", blocksPerGrid.x, blocksPerGrid.y, threadsPerBlock);
	myersFermiKernel_CC20<<<blocksPerGrid, threadsPerBlock, 0, idStream>>>((d_qryEntry_t *)qry->d_queries, ref->d_reference[idSupDev], cand->d_candidates, rebuff->d_reorderBuffer,
