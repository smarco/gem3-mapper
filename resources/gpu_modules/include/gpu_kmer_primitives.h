...
#define GPU_KMER_MIN_ELEMENTS                2048  // MIN elements per buffer (related to the SM -2048th-)
...
typedef struct {
  uint32_t             numBases;
  uint32_t             numQueries;
  gpu_kmer_qry_entry_t *d_queries;
  gpu_kmer_qry_entry_t *h_queries;
  gpu_kmer_qry_info_t  *d_queryInfo;
  gpu_kmer_qry_info_t  *h_queryInfo;
} gpu_kmer_queries_buffer_t;

typedef struct {
  uint32_t             numAlignments;
  gpu_kmer_alg_entry_t *h_alignments;
  gpu_kmer_alg_entry_t *d_alignments;
} gpu_kmer_alignments_buffer_t;

typedef struct {
  uint32_t             numCandidates;
  gpu_kmer_cand_info_t *h_candidates;
  gpu_kmer_cand_info_t *d_candidates;
} gpu_kmer_candidates_buffer_t;
...
typedef struct {
  ...
  uint32_t                     maxError;
  gpu_kmer_queries_buffer_t    queries;
  gpu_kmer_candidates_buffer_t candidates;
  gpu_kmer_alignments_buffer_t alignments;
} gpu_kmer_buffer_t;
