...
gpu_fmi_entry_t* gpu_fmi_buffer_get_index_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
  return(mBuff->index->fmi.h_fmi);
}

gpu_fmi_search_seed_t* gpu_fmi_search_buffer_get_seeds_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
  return(mBuff->data.search.seeds.h_seeds);
}

gpu_sa_search_inter_t* gpu_fmi_search_buffer_get_sa_intervals_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
  return(mBuff->data.search.saIntervals.h_intervals);
}

gpu_fmi_decode_init_pos_t* gpu_fmi_decode_buffer_get_init_pos_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
  return(mBuff->data.decode.initPositions.h_initBWTPos);
}

gpu_fmi_decode_end_pos_t* gpu_fmi_decode_buffer_get_end_pos_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
  return(mBuff->data.decode.endPositions.h_endBWTPos);
}

gpu_fmi_decode_text_pos_t* gpu_fmi_decode_buffer_get_ref_pos_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
...
uint32_t gpu_fmi_search_buffer_get_max_seeds_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
  return(mBuff->data.search.numMaxSeeds);
}

uint32_t gpu_fmi_decode_buffer_get_max_positions_(const void* const restrict fmiBuffer){
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
...
void gpu_fmi_search_init_buffer_(void* const restrict fmiBuffer)
{
  gpu_buffer_t* const restrict mBuff  = (gpu_buffer_t *) fmiBuffer;
...
void gpu_fmi_decode_reallocate_host_buffer_layout(gpu_buffer_t* const restrict mBuff)
{
  const void* rawAlloc = mBuff->h_rawData;
  //Adjust the host buffer layout (input)
...
void gpu_fmi_decode_reallocate_device_buffer_layout(gpu_buffer_t* const restrict mBuff)
{
  const void* rawAlloc = mBuff->d_rawData;
  //Adjust the host buffer layout (input)
...
void gpu_fmi_decode_init_buffer_(void* const restrict fmiBuffer)
{
  gpu_buffer_t* const restrict mBuff            = (gpu_buffer_t *) fmiBuffer;
  const size_t        sizeBuff         = mBuff->sizeBuffer * 0.95;
...
void gpu_fmi_decode_init_and_realloc_buffer_(void* const restrict fmiBuffer, const uint32_t numDecodes)
{
  gpu_buffer_t* const restrict mBuff                   = (gpu_buffer_t *) fmiBuffer;
  const uint32_t      idSupDevice             = mBuff->idSupportedDevice;
  const float         resizeFactor            = 2.0;
...
uint32_t gpu_fmi_search_print_buffer(const void* const restrict fmiBuffer)
{
  if(flag_print == 0){
    gpu_buffer_t* const restrict mBuff  = (gpu_buffer_t *) fmiBuffer;
...
gpu_error_t gpu_fmi_search_transfer_GPU_to_CPU(gpu_buffer_t* const restrict mBuff)
{
...
void gpu_fmi_search_send_buffer_(void* const restrict fmiBuffer, const uint32_t numSeeds)
{
  gpu_buffer_t* const restrict mBuff  = (gpu_buffer_t *) fmiBuffer;
  const uint32_t idSupDevice = mBuff->idSupportedDevice;
...
void gpu_fmi_search_receive_buffer_(const void* const restrict fmiBuffer)
{
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
...
gpu_error_t gpu_fmi_decode_transfer_CPU_to_GPU(gpu_buffer_t* const restrict mBuff)
{
...
gpu_error_t gpu_fmi_decode_transfer_GPU_to_CPU(gpu_buffer_t* const restrict mBuff)
{
...
void gpu_fmi_decode_send_buffer_(void* const restrict fmiBuffer, const uint32_t numDecodings, const uint32_t samplingRate)
{
  gpu_buffer_t* const restrict mBuff  = (gpu_buffer_t *) fmiBuffer;
  const uint32_t idSupDevice = mBuff->idSupportedDevice;
...
void gpu_fmi_decode_receive_buffer_(const void* const restrict fmiBuffer)
{
  const gpu_buffer_t* const restrict mBuff = (gpu_buffer_t *) fmiBuffer;
