...
uint32_t buildCounters(fmi_buffer_t *fmi)
{
	...
	// for(idEntry = 0; idEntry < countersNumEntries; ++idEntry){
	// 	printf("[ENTRY: %lld]: ", idEntry);
	// 	for(idCounter = 0; idCounter < FMI_NUM_COUNTERS; ++idCounter){
	// 		printf("%c=[%3lld] ", decodeBase(idCounter), fmi->h_countersFMI[idEntry].counters[idCounter]);
	// 	}
	// 	printf("\n");
	// }
